...
exports.verifyEmail = async ({ query }) => {
  try {
    ...
    if (!token)
      return { status: 400, body: { message: "Token não informado." } };
...
exports.resendVerificationEmail = async ({ body }) => {
  try {
    ...
    if (!emailInstitucional)
      return { status: 400, body: { message: "E-mail é obrigatório." } };
    ...
    if (user.emailVerificado)
      return { status: 400, body: { message: "O e-mail já foi confirmado." } };
    if (
